...
async function main() {
  ...
  const generatingImages = ora("Generating metadata");
  generatingImages.color = "yellow";
  generatingImages.start();
  await generateOnlyMetadata();
  await writeFile("./data.json", JSON.stringify(partsToMerge));
  // await generateOnlyImages();
  // await pokusaj();
  // await sleep(2);
  generatingImages.succeed("All metadata generated!");
...
//   let noMoreMatches = 0;
//   let images = []; // LISTA DIJELOVA JEDNE SLIKE
//   let id = 0;
//   await generateWeightedTraits();
//   if (config.deleteDuplicates) { // AKO NE DOZVOLIMO DUPLIKATE
//     while (!Object.values(weightedTraits).filter(arr => arr.length == 0).length && noMoreMatches < 20000) { // Object SE ODNOSI NA WEIGHTEDTRAITS (MOZE UZIMATI KEYS ILI VALUES)
//       let picked = [];
//       order.forEach(id => {
...
//       if (existCombination(images)) { // *1* AKO JE DUPLIKAT PRESKOCI
//         noMoreMatches++;
//         images = [];
//       } else { // AKO NIJE DUPLIKAT
//         generateMetadataObject(id, images); // NAPRAVIO METADATU
//         noMoreMatches = 0;
//         order.forEach((id, i) => {
//           remove(weightedTraits[id], picked[i]);
//         });
//         seen.push(images);
//         const b64 = await mergeImages(images, { Canvas: Canvas, Image: Image }); // SASTAVIO SLIKU
//         await ImageDataURI.outputFile(b64, outputPath + `${id}.png`); // NAPRAVIO SLIKU
//         images = [];
//         id++;
//       }
//     }
//   } else { // AKO DOZVOLIMO DUPLIKATE (MOZDA SE OVO MOGLO NAPRAVITI SAMO DA ULAZI *1* AKO DOZVOLJAVAMO DUPLIKATE)
...
async function generateOnlyMetadata() {
  let noMoreMatches = 0;
  let images = []; // LISTA DIJELOVA JEDNE SLIKE
  let id = 0;
  await generateWeightedTraits();
  if (config.deleteDuplicates) {
    // AKO NE DOZVOLIMO DUPLIKATE
    while (
      !Object.values(weightedTraits).filter((arr) => arr.length == 0).length &&
      noMoreMatches < 20000
    ) {
      // Object SE ODNOSI NA WEIGHTEDTRAITS (MOZE UZIMATI KEYS ILI VALUES)
      let picked = [];
      order.forEach((id) => {
        let pickedImgId = pickRandom(weightedTraits[id]);
        picked.push(pickedImgId);
      ...
      partsToMerge.push(images);

      if (existCombination(images)) {
        // *1* AKO JE DUPLIKAT PRESKOCI
        noMoreMatches++;
        images = [];
      } else {
        // AKO NIJE DUPLIKAT
    ...
    // AKO DOZVOLIMO DUPLIKATE (MOZDA SE OVO MOGLO NAPRAVITI SAMO DA ULAZI *1* AKO DOZVOLJAVAMO DUPLIKATE)
    while (
      !Object.values(weightedTraits).filter((arr) => arr.length == 0).length
    ) {
      order.forEach((id) => {
...
function generateMetadataObject(id, images) {
  // GENERATES METADATA OBJECT FOR ONE IMAGE AND PUSHES IT ON METADATA FOR ALL IMAGES
  metaData[id] = {
